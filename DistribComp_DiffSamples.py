	...
	#---Loop over event ---#
		...
		for iev in inputNtuples:
			...
			for imu_gen in range(0,iev.nMcParticles_pc) :
				if abs(iev.type_mcParticle_pc[imu_gen]) == 13 and abs(iev.motherType_mcParticle_pc[imu_gen]) ==  1000006 and abs(iev.d0BeamSpot_mcParticle_pc[imu_gen])<10: # gen muon
					hg_mud0_eff=inputEfficiency.Get(histnames[0])
					h_mupt_eff=inputEfficiency.Get(histnames[2])
					mud0_value = abs(iev.d0BeamSpot_mcParticle_pc[imu_gen])
					mupt_value = iev.pt_mcParticle_pc[imu_gen]
					...
					#print "W_mud0",W_mud0,"d0_value = ",mud0_value
					W_mupt = h_mupt_eff.GetEfficiency(Bin_mupt_value)
					continue
	

			for iele_gen in range(0,iev.nMcParticles_pc):
				if abs(iev.type_mcParticle_pc[iele_gen]) == 11 and abs(iev.motherType_mcParticle_pc[iele_gen]) ==  1000006 and abs(iev.d0BeamSpot_mcParticle_pc[iele_gen])<10:
					hg_eled0_eff=inputEfficiency.Get(histnames[1])
					h_elept_eff=inputEfficiency.Get(histnames[3])
					eled0_value = abs(iev.d0BeamSpot_mcParticle_pc[iele_gen])
					elept_value = iev.pt_mcParticle_pc[iele_gen]
				...
				W_event = W_mud0*W_mupt*W_eled0*W_elept # Total weight of the corresponding event (contribution of the parametrization eff. curves)
				for imu_gen in range(0,iev.nMcParticles_pc): # Give weight to each generated event
...
# -------------------------------------------------------- Filling histograms with reco/selected particles with FULL CMS SIMULATION -----------------------------+
		...
		for iev in chh:
			...
			for imu in range(0,iev.nMcParticles) :
				if abs(iev.type_mcParticle[imu]) == 13 and abs(iev.motherType_mcParticle[imu]) ==  1000006:
				#if (iev.nMuons==1):
					h_mupt.Fill(iev.pt_mcParticle[imu],1.000000001)
					h_mueta.Fill(iev.eta_mcParticle[imu],1.000000001)
					h_mud0.Fill(abs(iev.d0BeamSpot_mcParticle[imu]),1.000000001)

			for iele in range(0,iev.nMcParticles):
				if abs(iev.type_mcParticle[iele]) == 11 and abs(iev.motherType_mcParticle[iele]) ==  1000006:
					h_eleeta.Fill(iev.eta_mcParticle[iele],1.000000001)
					h_elept.Fill(iev.pt_mcParticle[iele],1.000000001)
					h_eled0.Fill(abs(iev.d0BeamSpot_mcParticle[iele]),1.000000001)
...
#---------------------------------------------------------------------- Histograms comparison ----------------------------------------------------------------------+
		...
		t3.Update()
		t3.Print("RatioDistribComp_BEAMSPOT_m"+str(Mass)+"_Ctau"+ str(Ctau)+"_mupt.pdf")
		...
		t3.Update()
		t3.Print("RatioDistribComp_BEAMSPOT_m"+str(Mass)+"_Ctau"+ str(Ctau)+"_mud0.pdf")
		...
		t3.Update()
		t3.Print("RatioDistribComp_BEAMSPOT_m"+str(Mass)+"_Ctau"+ str(Ctau)+"_elept.pdf")
		...
		t3.Update()
		t3.Print("RatioDistribComp_BEAMSPOT_m"+str(Mass)+"_Ctau"+ str(Ctau)+"_eled0.pdf")
