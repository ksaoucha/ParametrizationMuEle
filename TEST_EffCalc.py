...
for Mass in [500]: #Looping over samples with different masses
	for Ctau in [100]: #Looping over samples with different lifetimes for a certain mass
		ch  = ROOT.TChain("preCutTree","preCutTree")		
...
# Loop over muons - fill in lorentz vector and fill some histograms
			...
			for i_gen in range(0,iev.nMcParticles_pc) : # generated particles
				if abs(iev.motherType_mcParticle_pc[i_gen]) ==  1000006:
					jj+=1
					if abs(iev.type_mcParticle_pc[i_gen]) == 13 : # Condition that checks if the lepton is a muon AND if it's coming from a sTop
						lvmu.SetPtEtaPhiE(iev.pt_mcParticle_pc[i_gen],iev.eta_mcParticle_pc[i_gen],iev.phi_mcParticle_pc[i_gen],iev.E_mcParticle_pc[i_gen])
						passedCut_muon = False
						if iev.pt_mcParticle_pc[i_gen]>10 and iev.v0_mcParticle_pc[i_gen]<4 and abs(iev.vz_mcParticle_pc[i_gen])<30 and abs(iev.d0BeamSpot_mcParticle_pc[i_gen])>0.02: # Condition that checks if the generated particle has a pt>10 and is inside the detection region
							passedCut_muon = True
						if passedCut_muon == False:
							continue
						hg_mupt.Fill(iev.pt_mcParticle_pc[i_gen])
						hg_mueta.Fill(iev.eta_mcParticle_pc[i_gen])
						hg_mud0.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen])) # Filling the histograms with events that passed the pre-selection
						...
						if passedCut_muon==True:
							for imu_reco in range(0,iev.nMuons_pc):
								...
								if (lvmu.DeltaR(lvmu_reco)>0.05): # Condition: the reco and the gen particle must be close to each others
									continue
								hg_mupt_cut.Fill(iev.pt_mcParticle_pc[i_gen])
								hg_mueta_cut.Fill(iev.eta_mcParticle_pc[i_gen])
								hg_mud0_cut.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen])) # Filling close reconstructed events in histograms 
								...
								if passedCut_id_iso_muon==True:
									...
									hg_mupt_cutIso.Fill(iev.pt_mcParticle_pc[i_gen])	
									hg_mueta_cutIso.Fill(iev.eta_mcParticle_pc[i_gen])	
									hg_mud0_cutIso.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen]))
								break		
...
# ----------------------------------------------------------------- ELECTRONS ---------------------------------------------------------------------------------------
	...
	# Loop over electrons - fill in lorentz vector and fill some histograms
					...
					if abs(iev.type_mcParticle_pc[i_gen]) == 11:
						lvele.SetPtEtaPhiE(iev.pt_mcParticle_pc[i_gen],iev.eta_mcParticle_pc[i_gen],iev.phi_mcParticle_pc[i_gen],iev.E_mcParticle_pc[i_gen])
				
						passedCut_electron = False
						if iev.pt_mcParticle_pc[i_gen]>10 and iev.v0_mcParticle_pc[i_gen]<4 and abs(iev.vz_mcParticle_pc[i_gen])<30 and abs(iev.d0BeamSpot_mcParticle_pc[imu_gen])>0.02:
							passedCut_electron = True
						if passedCut_electron == False:
							continue
						hg_eleeta.Fill(iev.eta_mcParticle_pc[i_gen])
						hg_elept.Fill(iev.pt_mcParticle_pc[i_gen])
						hg_eled0.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen]))
						...
						if passedCut_electron == True:
							for iele_reco in range(0,iev.nElectrons_pc):
								...
								if (lvele.DeltaR(lvele_reco)>0.05):
									continue
								hg_eleeta_cut.Fill(iev.eta_mcParticle_pc[i_gen])
								hg_elept_cut.Fill(iev.pt_mcParticle_pc[i_gen])
								hg_eled0_cut.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen]))
								...
								if passedCut_id_iso_electron==True:
									...
									hg_elept_cutIso.Fill(iev.pt_mcParticle_pc[i_gen])	
									hg_eleeta_cutIso.Fill(iev.eta_mcParticle_pc[i_gen])	
									hg_eled0_cutIso.Fill(abs(iev.d0BeamSpot_mcParticle_pc[i_gen]))
								break
